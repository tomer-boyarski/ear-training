...
class Sequencer(object):
    ...
    def event_write(self, event, direct=False, relative=False, tick=False):
        ...
        ## Note Event
            ...
            seqev.data.question.channel = event.channel
            seqev.data.question.question = event.pitch
            seqev.data.question.velocity = event.velocity
        ## Control Change
    ...
    def event_read(self):
        ...
        if ev and ev.type in (S.SND_SEQ_EVENT_NOTEON, S.SND_SEQ_EVENT_NOTEOFF):
            if ev.type == S.SND_SEQ_EVENT_NOTEON:
                ...
                mev.channel = ev.data.question.channel
                mev.pitch = ev.data.question.question
                mev.velocity = ev.data.question.velocity
