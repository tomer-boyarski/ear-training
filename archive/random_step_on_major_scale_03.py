...
step_size = 1
last_sample_weight = 0.2

response_time_value_for_error = np.nan

lower_bound_response_time = 1
upper_bound_response_time = 1.3
...
while not Termination_Flag:
    ...
    question_index += 1
    max_step_size_list.append(step_size)
    CorrectAnswer = False
    ...
    while not CorrectAnswer and not Termination_Flag:
        ...
        if identified_note == new_note_name:  # or idendtified_note == str(note_number-notes_numbers_to_play_now[0]+1):\
            ...
            autoregressive_response_time.append(
                0.9 * autoregressive_response_time[-1] + 0.1 * response_time_now)
            if autoregressive_response_time[-1] < lower_bound_response_time:
                step_size += 1
            elif autoregressive_response_time[-1] > upper_bound_response_time and step_size>0:
                step_size -= 1
...
if True:
    ...
    x_ticks_positions = np.arange(0, question_index)
    my_label_response_time = 'autoregressive response time'
    ax.plot(x_ticks_positions,
            autoregressive_response_time[1:],
            ':', label=my_label_response_time)
    ...
    ax.plot(x_ticks_positions,
            0.7 * np.ones(question_index),
            '-', label='minimal response time')
    ax.plot(x_ticks_positions,
            lower_bound_response_time * np.ones(question_index),
            '-', label='response time lower bound')
    ax.plot(x_ticks_positions,
            upper_bound_response_time * np.ones(question_index),
            '-', label='response time upper bound')
