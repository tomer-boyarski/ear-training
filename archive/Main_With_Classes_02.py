...
num = 10000
for _ in range(num):
    mylist.append(Question(7, 7))
    mylist[-1].get_step_size_discrete_levels()
    mylist[-1].get_step_direction_and_new_note()
...
ax[0].bar(values, counts / np.sum(counts))
ax[0].title.set_text('step_size_list')
values, counts = np.unique(step_direction_list, return_counts=True)
ax[1].bar(values, counts / np.sum(counts))
ax[1].title.set_text('step_direction_list')
values, counts = np.unique(new_note_index_list, return_counts=True)
ax[2].bar(values, counts / np.sum(counts))
ax[2].title.set_text('new_note_index_list')
