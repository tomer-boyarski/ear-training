...
class Question:
    def __init__(self, note=None):
        self.step = Step()
        self.note = Note() if note is not None else note
        self.response = Response()
        self.error_state = initial.error_state
        self.Termination_Flag = False
        self.volume = Volume()

    def phase_1(self, question_list):
        self._set_difficulty(question_list)
        self._set_notes(self, question_list)
        self.set_volume(question_list)
        self._play_note()

    def _set_notes(self, question_list):
        if not question_list[-1].response.type:
            ...
            self._set_step_size_if_no_recent_errors()
            self._set_step_trend_and_new_note_index_if_no_recent_errors(question_list)
    ...
    def _set_step_size_if_no_recent_errors(self):
    ...
    def _set_step_trend_and_new_note_index_if_no_recent_errors(self, question_list):
        while True:
...
while not list_of_questions[-1].Termination_Flag:
    question = Question()
    question.phase_1(list_of_questions)
